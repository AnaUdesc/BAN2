...
public class AnimaisController {
    ...
    public void createAnimais(Driver driver) {
        ...
        ClientesNeo4jModel.listarClientesDisponiveis(driver);
        ...
        try (Session session = driver.session()) {
            ...
            AnimaisNeo4jModel.create(mb, session);
    ...
    void listarAnimaisClientes(Driver driver) {
        try (Session session = driver.session()) {
            HashSet all = AnimaisNeo4jModel.listAllWithClientes(session);
            Iterator<AnimaisBean> it = all.iterator();
            while(it.hasNext()) {
                System.out.println(it.next().toString());
            }
